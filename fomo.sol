...
contract FoMo3Dlong is modularLong {
     ...
     */
    modifier isWithinLimits(uint256 _eth) { // 1 eth = 10^18 wei
        require(_eth >= 1000000000, "pocket lint: not a valid currency"); // 10^9 wei = 10^(-9) eth
        require(_eth <= 100000000000000000000000, "no vitalik, no"); // 10^23 wei = 10^5 eth
        _;    
    }
    ...
    {  
        ...
        if (_now > round_[_rID].strt + rndGap_ && (_now <= round_[_rID].end || (_now > round_[_rID].end && round_[_rID].plyr == 0)))
            ...
            return ( 75000000000000 ); // init = 75 * 10^12 wei
    }
        ...
        private
    {
        ...
        if (_eth > 1000000000) // 10^9 wei
        {
            ...
            if (_keys >= 1000000000000000000)
            {
                ...
                // set new leaders 
                if (round_[_rID].plyr != _pID) // ## the last active user ##
                    round_[_rID].plyr = _pID;  
                if (round_[_rID].team != _team) // ## the last active team ##
            ...
            // manage airdrops
            if (_eth >= 100000000000000000) // 10^17 wei = 0.1 wth
            {
                airDropTracker_++;
                if (airdrop() == true)
                {
                    // gib muni
                    uint256 _prize;
                    if (_eth >= 10000000000000000000) // 10 eth
                    {
                    ...
                    } else if (_eth >= 1000000000000000000 && _eth < 10000000000000000000) { // 1 ~ 10 eth
                        // calculate prize and give it to winner
        ...
        returns (F3Ddatasets.EventReturns)
    {
        ...
        uint256 _win = (_pot.mul(48)) / 100;                                // winner share = 48%
        uint256 _com = (_pot / 50);                                         // community rewards = 2%
        uint256 _gen = (_pot.mul(potSplit_[_winTID].gen)) / 100;            // gen share = 48%
        uint256 _p3d = (_pot.mul(potSplit_[_winTID].p3d)) / 100;            // p3d share = 48%
        uint256 _res = (((_pot.sub(_win)).sub(_com)).sub(_gen)).sub(_p3d);  // amount reserved for next pot = ...%
